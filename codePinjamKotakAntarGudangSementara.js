...
function saveDataPinjamKotakAntarGudangSementara(idKotakDipinjam, tujuanPeminjaman, gudangTujuan) {
  ...
  idKotakDipinjam.forEach(id => {
    ...
    sheetPinjamKotak.getRange(lrPinjamKotak+1,4).setValue(sheetKotak.getRange(dataRow,8).getValue())
    sheetPinjamKotak.getRange(lrPinjamKotak+1,5).setValue(gudangTujuan)
    sheetPinjamKotak.getRange(lrPinjamKotak+1,6).setValue('FALSE')
    sheetPinjamKotak.getRange(lrPinjamKotak+1,7).setValue(user)
    sheetPinjamKotak.getRange(lrPinjamKotak+1,8).setValue(Utilities.formatDate(date,"Asia/Bangkok","dd MMMM yyyy HH:mm:ss"))
