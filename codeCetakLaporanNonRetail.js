...
function clickBtnGenerateReportNonRetail(formData) {
  const params = {
    periode: formData.periode
  }
  PropertiesService.getUserProperties().setProperty('paramTriggerReportNonRetail', JSON.stringify(params))

  ScriptApp.newTrigger('printReportNonRetail')
    .timeBased()
    .after(5*1000)
    .create()
...
function printReportNonRetail(e) {
  ...
  var sheetTemplate = spreadsheet.getSheetByName('Template Non Retail')
  ...
  var dataFile = generateAndSendLink(`Laporan KPS Non Retail Periode - ${periode}`, 'Template Non Retail')
  ...
  PropertiesService.getUserProperties().deleteProperty('paramTriggerReportNonRetail')
