function showCetakLaporanRetail() {
  var formCetakLaporanRetail = HtmlService
    .createHtmlOutputFromFile('formCetakLaporanRetail')
    .setWidth(400)
    .setHeight(200)
...
function printReportRetail(e) {
  ...
  //generate header
  sheetTemplate.getRange(1,1).setValue('Laporan KPS Retail')
  ...
  var filteredKotak = dataKotak.filter((kotak) => {
      ...
      kotak.periode.forEach((data) => {
          ...
          res = res || (parseInt(periode)===parseInt(boxPeriode) && kotak.jenisDokumen==='Retail' && kotak.status==='Terpakai')
      })
      return res
  })
